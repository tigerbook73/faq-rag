...
将当前 `src/lib/route-policy.ts` 迁移为目录模块：

```txt
src/lib/route-policy/
  constants.ts
  types.ts
  match.ts
  public.ts
...
调用方继续从 `@/lib/route-policy` 导入（路径不变）。

模块职责：

| 文件           | 职责                                                       |
| -------------- | ---------------------------------------------------------- |
| `constants.ts` | `USER_HOME_PATH`、`ADMIN_HOME_PATH`、`SIGN_IN_PATH` 等常量 |
| `types.ts`     | `RouteAccess`、`RoutePolicy` 类型                          |
| `match.ts`     | exact/prefix 匹配、policy 查找通用函数                     |
| `public.ts`    | public routes 与 public API bypass 定义                    |
| `user.ts`      | user private routes、user API 定义                         |
| `admin.ts`     | admin private routes、admin API 定义                       |
| `redirect.ts`  | redirect sanitizer、role-aware redirect 计算               |
| `index.ts`     | 组合 policy、导出统一 helper                               |
...
建议将 Supabase client 文件按职责拆分（不改变 session 行为）：

```txt
src/lib/supabase/
  config.ts     ← URL/key 读取
  cookies.ts    ← Next cookies adapter（getAll / setAll）
  server.ts     ← createSupabaseServerClient()
  service.ts    ← service-role client（已有）
```
...
src/lib/auth/
  ...
  profiles.ts   ← getProfile()、profile 查询逻辑
  user.ts       ← requireUser()
...
- 将 `src/lib/route-policy.ts` 迁移为 `src/lib/route-policy/` 目录模块。
- 添加 `resolvePostLoginRedirect(role, from)`。
- 更新路由策略测试。

### Phase 2: Auth Helper 与 Supabase client 整理

- 拆分 `src/lib/auth/` 文件。
- 确认 `requireUser()` 接受 role=admin。
- `GET /api/auth/me` 补充 `role` 字段。
- Supabase client 文件按职责拆分。
