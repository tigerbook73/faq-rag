...
**建议下一步**：PR 合并后执行剩余收尾——`/code-review` 整体审查、归并 5 条 [arch] 条目到架构文档、决定 `plans/mobile-chat/` 去留。
