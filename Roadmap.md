...
| 类别                                            | 项目数 | 完成 | 建议先做                          |
| ----------------------------------------------- | ------ | ---- | --------------------------------- |
| [RAG 核心质量](#一rag-核心质量)                 | 6 项   | 3 ✅ | Cross-encoder Rerank、HyDE 检索   |
...
const parser = createParser((event) => { /* handle */ });
for await (const chunk of reader) { parser.feed(decoder.decode(chunk)); }
```
