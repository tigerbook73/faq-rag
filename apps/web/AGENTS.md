...
- **`fileRef` 字段**:Prisma 字段 `fileRef` 映射到数据库列 `file_path`,存 Supabase Storage 对象路径(`embed/{docId}/{sanitizedFilename}`)。`storage/index.ts` 中的 `readUploadedFile` 直接读取 Supabase Storage(无本地文件系统分支)。CLI 摄取(`ingestFile`)例外 —— 它把本地磁盘路径写入 `fileRef`,该路径不会被 `readUploadedFile` 读取,仅用于一次性建库。
